...
from src.loss import PixLoss, ClsLoss
from src.dataset import MyData, CarSegmentationData
from src.models.birefnet import BiRefNet
from src.utils import Logger, AverageMeter, set_seed, check_state_dict
...
if os.path.exists(os.path.join(config.data_root_dir, args.testsets.strip('+').split('+')[0])):
...
def init_data_loaders(to_be_distributed):
    ...
    train_loader = prepare_dataloader(
        CarSegmentationData(datasets=args.trainset, image_size=config.size, is_train=True),
        config.batch_size, to_be_distributed=to_be_distributed, is_train=True
    )
    print(len(train_loader), "batches of train dataloader {} have been created.".format(args.trainset))
    test_loaders = {}
    for testset in args.testsets:
        _data_loader_test = prepare_dataloader(
            CarSegmentationData(datasets=testset, image_size=config.size, is_train=False),
