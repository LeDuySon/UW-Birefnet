...
parser.add_argument("--experiment_name", default="BiRefNet", type=str)
...
def init_data_loaders(to_be_distributed):
    ...
    train_loader = prepare_dataloader(
        CarSegmentationData(
            datasets=args.trainset, image_size=config.size, is_train=True
        ),
        config.batch_size,
    ...
    for testset in args.testsets:
        _data_loader_test = prepare_dataloader(
            CarSegmentationData(
                datasets=testset, image_size=config.size, is_train=False
            ),
...
class Trainer:
    def __init__(
        ...
        if config.out_ref:
            self.criterion_gdt = (
                nn.BCELoss() if not config.use_fp16 else nn.BCEWithLogitsLoss()
            )
    ...
    def _load_adv_components(self):
        ...
        if config.optimizer == "AdamW":
            optimizer_d = optim.AdamW(
                params=disc.parameters(), lr=config.lr, weight_decay=1e-2
            )
        elif config.optimizer == "Adam":
            optimizer_d = optim.Adam(
                params=disc.parameters(), lr=config.lr, weight_decay=0
            )
        lr_scheduler_d = torch.optim.lr_scheduler.MultiStepLR(
            optimizer_d,
            milestones=[
                lde if lde > 0 else args.epochs + lde + 1
                for lde in config.lr_decay_epochs
            ],
            gamma=config.lr_decay_rate,
        )
    ...
    def validate_model(self, epoch):
        ...
        for testset, data_loader_test in self.test_loaders.items():
            ...
            performance_dict = valid(
                ...
                pred_dir=".",
                method=(
                    args.ckpt_dir.split("/")[-1]
                    if args.ckpt_dir.split("/")[-1].strip(".").strip("/")
                    else "tmp_val"
                ),
                testset=testset,
